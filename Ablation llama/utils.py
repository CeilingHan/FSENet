...
def gaussian_smoothing(S, r=0.25):
    ...
    return smoothed_S
def create_multi_step_labels(point_labels, step_width=5, peak_value=0.9, base_value=0.5):
    B, T, _ = point_labels.shape
    device = point_labels.device
    C = point_labels.shape[-1] - 1
    step_labels = torch.full((B, T, C+1), 0.1, device=device)
    
    step_labels[..., -1] = 1.0
    
    for b in range(B):
        for t in range(T):
            cls_indices = torch.where(point_labels[b, t, :-1] > 0)[0]
            ...
            for cls_idx in cls_indices:
                ...
                step_labels[b, start:end,:] = 0.1
                step_labels[b, start:end, cls_idx]=full_weights
                step_labels[b, start:end, -1] = 1.0 - full_weights  # 背景通道互补
