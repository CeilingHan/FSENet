...
def save_tensor(tensor, dir_name, vid_name, prefix, fmt="%.6f"):
    arr = tensor.detach().cpu().numpy()
    
    # 自动维度压缩 (移除批处理维度)
    if arr.ndim >= 3 and arr.shape[0] == 1:  # 假设首维为batch维度
        arr = arr.squeeze(0)
    if arr.ndim not in (1, 2):
        raise ValueError(f"不支持的张量维度 {arr.ndim}，期望1D或2D数据")
    os.makedirs(dir_name, exist_ok=True)
    save_path = os.path.join(dir_name, f"{vid_name}_{prefix}.txt")
    np.savetxt(save_path, arr, fmt=fmt)
    print(f"[{prefix}] 特征保存至 {save_path} (shape: {arr.shape})")
def test(net, config, logger, test_loader, test_info, step, model_file=None):
    with torch.no_grad():
        ...
        for i in range(len(test_loader.dataset)):
            ...
            vid_score, cas_sigmoid_fuse, _ ,sen= net(_data)

            if torch.isnan(cas_sigmoid_fuse).any() or torch.isinf(cas_sigmoid_fuse).any():
                print("NaN or Inf detected in cas_sigmoid_fuse")
            #video_name = vid_name[0]
            #save_tensor(cas_sigmoid_fuse, "CAS_test", video_name, "cas")  
            ...
            score_np = vid_score[0].cpu().data.numpy() # C
        ...
        mAP, _, info = anet_detection.evaluate()
        cAP, _, Rc, F2 = info
        print("mAP",mAP, Rc, F2)

        for i in range(tIoU_thresh.shape[0]):
            logger.log_value('acc/mAP@{:.1f}'.format(tIoU_thresh[i]), mAP[i], step)
            logger.log_value('acc/Rc@{:.1f}'.format(tIoU_thresh[i]), Rc[i], step)

        logger.log_value('acc/Average mAP[0.1:0.3]', mAP[:5].mean(), step)
        logger.log_value('acc/Average nAP[0.1:0.3]', cAP[0], step)
        logger.log_value('acc/Average pAP[0.1:0.3]', cAP[1], step)

        test_info["step"].append(step)
        test_info["average_mAP[0.1:0.3]"].append(mAP[:5].mean())
        test_info["average_nAP[0.1:0.3]"].append(cAP[0])
        test_info["average_pAP[0.1:0.3]"].append(cAP[1])

        for i in range(tIoU_thresh.shape[0]):
            test_info["mAP@{:.2f}".format(tIoU_thresh[i])].append(mAP[i])
            test_info["Rc@{:.2f}".format(tIoU_thresh[i])].append(Rc[i])
            test_info["F2@{:.2f}".format(tIoU_thresh[i])].append(F2[i])
